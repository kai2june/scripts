...
do
    if [ ${tool} = "rsem" ]
        then
            ...
            jimmy=${jimmy}+`awk 'NR==2' ${basepath}${x}/${tool}.${index}/${tool}.${index}.quant.stat/computeTPM/correlation.txt | awk '{printf($2" ")}'`
        ...
        then
            ...
            jimmy=${jimmy}+`awk 'NR==2' ${basepath}${x}/${tool}.${index}/${tool}.${index}.quant/computeTPM/correlation.txt | awk '{printf($2" ")}'`
    elif [ ${tool} = "jimmy_salmon" -o ${tool} = "salmon" ]
        then
            echo "${basepath}${x}/${tool}.${index}/${tool}.${index}.${branch}.${mode}quant/computeTPM/correlation.txt"
            jimmy=${jimmy}+`awk 'NR==2' ${basepath}${x}/${tool}.${index}/${tool}.${index}.${branch}.${mode}quant/computeTPM/correlation.txt | awk '{printf($2" ")}'`
    fi
done
